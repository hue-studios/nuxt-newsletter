...
export const useNewsletter = () => {
  ...
  const fetchNewsletter = async (id: number) => {
    try {
      ...
      const response = await directus.request(
        (readItem as any)("newsletters", id, {
          fields: [
            "*",
            "blocks.id",
            "blocks.sort",
  ...
  const updateNewsletter = async (id: number, data: Partial<Newsletter>) => {
    try {
      ...
      const response = await directus.request(
        (updateItem as any)("newsletters", id, data)
      );
...
export const useNewsletterBlocks = () => {
  ...
  const updateBlock = async (id: number, data: Partial<NewsletterBlock>) => {
    try {
      ...
      const response = await directus.request(
        (updateItem as any)("newsletter_blocks", id, data)
      );
  ...
  const reorderBlocks = async (newsletterId: number, blockIds: number[]) => {
    try {
      ...
      const updates = blockIds.map((id, index) =>
        directus.request(
          (updateItem as any)("newsletter_blocks", id, { sort: index + 1 })
        )
      );
  ...
  const createFromTemplate = async (
    ...
    try {
      ...
      await directus.request(
        (updateItem as any)("newsletter_templates", template.id, {
